export default function ResultCard({ data }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Risk Badge */}
      <div className={`${risk.bg} ${risk.border} border rounded-lg px-3 sm:px-4 py-2 sm:py-3 inline-flex items-center gap-2`}>
        <div className={`w-2 h-2 rounded-full ${risk.color.replace('text', 'bg')}`}></div>
        <span className={`text-xs sm:text-sm font-semibold ${risk.color}`}>
